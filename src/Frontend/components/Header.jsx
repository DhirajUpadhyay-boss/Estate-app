...
const Navbar = () => {
  ...
  return (
    <div className="fixed top-0 left-0 w-full z-50 bg-gradient-to-r from-blue-900 via-gray-900 to-black shadow-lg">
      <div className="container mx-auto flex items-center justify-between py-4 px-4 sm:px-6 md:px-10 lg:px-20">
        {/* Logo */}
        <Link to="/">
          <img
            src={assets.logo}
            alt="Estate Logo"
            className="w-28 sm:w-32 md:w-40 transition-transform hover:scale-105"
          />
        </Link>

        {/* Center navigation (desktop only) */}
        <ul className="hidden md:flex gap-4 lg:gap-8 text-white font-semibold items-center">
          {/* Home */}
          <li>
            <NavLink
              to="/"
          ...
          {/* Property Trends dropdown */}
          <li className="relative">
            <button
              type="button"
              onClick={() => setShowTrendsDropdown((prev) => !prev)}
        ...
        {/* Right side: Login/Register or Profile chip (desktop only) */}
        <div className="hidden md:flex items-center gap-4 min-h-[44px]">
          {authLoading && (
            <div className="w-10 h-10 rounded-full bg-white/20 animate-pulse" aria-hidden />
          )}
        ...
        {/* Mobile: right side — profile chip or hamburger */}
        <div className="flex md:hidden items-center gap-3">
          {!authLoading && isLoggedIn && user && (
            <Link
              to="/profile"
              className="flex items-center bg-white rounded-full px-2 py-1 shadow-md"
            >
